"""
image_tools/ocr_tools.py

Scans the corners of an image for a printed/imprinted date -- the kind
old date-stamp cameras burn directly into a photo, or that a scanned
print carries from the original film. Migrated here from the standalone
ocr_date/ folder, unchanged in behavior.

This is deliberately opt-in, not part of analyze_date's automatic
per-file signal gathering -- it's slow (scanning multiple corners at
multiple rotations and preprocessing variants adds up) and only useful
for files that already have weak or no other evidence. It's meant to be
invoked explicitly (directly, or eventually via the analyze_date CLI's
...
# confident and silently incorrect.
...
def find_date_in_corners(file_path, debug_dir=None):
    ...
    to OCR for every corner/rotation/variant combination actually tried,
    as PNG files named '<original filename>_<corner>_rot<degrees>_<variant>.png'.

    Returns a dict:
        {'date': datetime or None, 'corner': str or None, 'rotation': int or None,
         'variant': 'raw' or 'otsu' or None, 'raw_text': str or None,
         'ocr_confidence': float or None,
         'checked': [{'corner': str, 'rotation': int, 'variant': str,
                       'raw_text': str, 'ocr_confidence': float}, ...]}
    """
    ...
    for corner_name in check_order:
        ...
        for rotation in ROTATIONS_TO_TRY:
            ...
            # always better, so both get a chance rather than committing
            # to one.
